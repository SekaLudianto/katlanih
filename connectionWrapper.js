...
class TikTokConnectionWrapper extends EventEmitter {
    constructor(uniqueId, options, enableLog) {
        ...
        this.connection.on('streamEnd', (msg) => {
            this.log(`streamEnd event received, giving up connection. Action: ${msg.action}`);
            this.reconnectEnabled = false;
            this.emit('streamEnd', msg);
        });

        this.connection.on('disconnected', () => {
            if (this.reconnectEnabled) globalConnectionCount -= 1;
    ...
    scheduleReconnect(reason) {
        if (this.clientDisconnected || !this.reconnectEnabled) {
            return;
        }
        ...
        setTimeout(() => {
            if (this.clientDisconnected || !this.reconnectEnabled || this.reconnectCount >= this.maxReconnectAttempts) {
                return;
            }
    ...
    disconnect() {
        this.log(`Client connection disconnected, terminating TikTok connection.`);
